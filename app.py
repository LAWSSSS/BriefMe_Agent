...
#                     )
#                 # ------- 下载图片包 -------
#                 with gr.Accordion("📦 下载图片包", open=True):
#                     download_zip_file = gr.File(label="点击下载 ZIP 文件到本地", interactive=False)

#                 # ------- 下载实时日志面板 -------
#                 with gr.Accordion("📋 下载实时日志", open=True):
...
#                         js="() => { setTimeout(() => { const el = document.querySelector('#log_textbox textarea'); if(el) el.scrollTop = el.scrollHeight; }, 100); }"
#                     )
...
#             # 检查是否是下载指令的返回（包含 zip_file）
#             if isinstance(reply, dict):
#                 zip_file = reply.get("zip_file")
#                 reply_text = reply.get("summary_text", str(reply))
#                 history.append({"role": "assistant", "content": reply_text})
#                 xlsx, pptx, imgs = _scan_latest_artifacts()
#                 return history, session, xlsx, pptx, imgs, zip_file
#             else:
#                 history.append({"role": "assistant", "content": str(reply)})
#                 xlsx, pptx, imgs = _scan_latest_artifacts()
#                 return history, session, xlsx, pptx, imgs, None
    ...
    with gr.Blocks(
        ...
        with gr.Row():
            ...
            with gr.Column(scale=4):
                ...
                # ------- 下载图片包 -------
                with gr.Accordion("📦 下载图片包", open=False):  # 只改了这里：True -> False
                    download_zip_file = gr.File(label="点击下载 ZIP 文件到本地", interactive=False)
...
# --- 绑定：输入/按钮 ---
        ...
        def _sync_respond(_pending_message, history, session):
            ...
            # 检查是否是下载指令
            if "下载盛隆工厂" in pending_message and "监控图像" in pending_message:
                import re
                ...
                # 解析日期
                match = re.search(r'(\d{4}-\d{2}-\d{2})\s*到\s*(\d{4}-\d{2}-\d{2})', pending_message)
                if match:
